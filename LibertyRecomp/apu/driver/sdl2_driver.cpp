...
static void AudioThread() {
  ...
  LOG_WARNING("[AUDIO_THREAD] Starting audio thread with GuestThreadContext");
  GuestThreadContext ctx(0);
  LOG_WARNING("[AUDIO_THREAD] GuestThreadContext created successfully");
  ...
  while (!g_audioThreadShouldExit) {
    ...
    if ((queuedAudioSize / callbackAudioSize) <= MAX_LATENCY) {
      s_callbackCount++;
      if (s_callbackCount <= 10 || s_callbackCount % 100 == 0) {
        LOGF_WARNING(
            "[AUDIO_THREAD] Invoking guest callback #{} param=0x{:08X}",
            s_callbackCount, g_clientCallbackParam);
      }
    ...
    constexpr auto INTERVAL =
        1000000000ns * XAUDIO_NUM_SAMPLES / XAUDIO_SAMPLES_HZ;
    auto next = now + (INTERVAL - now.time_since_epoch() % INTERVAL);

    std::this_thread::sleep_for(
        std::chrono::floor<std::chrono::milliseconds>(next - now));

    while (std::chrono::steady_clock::now() < next)
      std::this_thread::yield();
  }
  LOG_WARNING("[AUDIO_THREAD] Audio thread exiting");
}
...
void XAudioRegisterClient(PPCFunc *callback, uint32_t param) {
  LOGF_WARNING("[AUDIO] XAudioRegisterClient: callback=0x{:p} param=0x{:08X}",
               (void *)callback, param);
