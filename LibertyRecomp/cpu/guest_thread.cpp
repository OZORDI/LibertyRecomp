...
// Legacy constants for backward compatibility (now defined in guest_thread.h as
// X360_*)
...
GuestThreadContext::GuestThreadContext(uint32_t cpuNumber) {
  ...
  // Use physical heap (0x80000000+) so stack addresses match Xbox 360
  // expectations
  ...
  uint32_t stackBase = g_memory.MapVirtual(
      thread + TOTAL_SIZE); // High address (stack grows down)
  uint32_t stackLimit =
      g_memory.MapVirtual(thread + X360_STACK_OFFSET); // Low address
  ...
  if (deviceAddr != 0) {
    ...
    *reinterpret_cast<uint32_t *>(tlsMem + TLS_DEVICE_OFFSET) =
        __builtin_bswap32(deviceAddr);
  }
  ...
  // -------------------------------------------------------------------------
  teb->thread_id =
      GuestThread::GetCurrentThreadId(); // 0x14C - Thread identifier
...
GuestThreadContext::~GuestThreadContext() { g_userHeap.Free(thread); }
...
#endif
  // FIX: Memory barrier to ensure all fields written before thread start are
  // visible
...
GuestThreadHandle *GuestThread::Start(const GuestThreadParams &params,
                                      uint32_t *threadId) {
...
#ifdef _WIN32
  GuestThread::SetThreadName(threadId,
                             (const char *)g_memory.Translate(ByteSwap(*name)));
#endif
}

int GetThreadPriorityImpl(GuestThreadHandle *hThread) {
#ifdef _WIN32
  return GetThreadPriority(hThread == GetKernelObject(CURRENT_THREAD_HANDLE)
                               ? GetCurrentThread()
...
uint32_t SetThreadIdealProcessorImpl(GuestThreadHandle *hThread,
                                     uint32_t dwIdealProcessor) {
  return 0;
}
...
// GUEST_FUNCTION_STUB(sub_82BD58F8); // Some function that updates the TEB,
// don't really care since the field is not set
