...
// Forward declarations of our hook functions - use _hook suffix to avoid weak
// symbol collision
extern "C" void
sub_829A9738_hook(PPCContext &ctx,
                  uint8_t *base); // Wait helper (NtWaitForSingleObjectEx)
extern "C" void sub_829A3560_hook(PPCContext &ctx, uint8_t *base);
extern "C" void sub_829A39A0_hook(PPCContext &ctx, uint8_t *base);
extern "C" void
sub_829A3238_hook(PPCContext &ctx,
                  uint8_t *base); // Sync wait function (KeWaitForSingleObject)
extern "C" void sub_829A39F0_hook(
    PPCContext &ctx,
    uint8_t *base); // Intermediate sync function (calls sub_829A3238)
extern "C" void sub_827DB338_hook(PPCContext &ctx, uint8_t *base);
extern "C" void sub_827DB988_hook(PPCContext &ctx, uint8_t *base);
// sub_827DACD8 uses direct PPC_FUNC override

extern "C" void
sub_829A2380_hook(PPCContext &ctx,
                  uint8_t *base); // Semaphore acquire (sync table)
extern "C" void
sub_829A21F8_hook(PPCContext &ctx,
                  uint8_t *base); // Semaphore create wrapper (sync table)
extern "C" void sub_829D8568_hook(PPCContext &ctx,
                                  uint8_t *base); // GPU command buffer flush
...
bool PatchFuncMapping(size_t guestAddr, PPCFunc *newFunc) {
  ...
  printf(
      "[PATCH] WARNING: Guest address 0x%08zX not found in PPCFuncMappings\n",
      guestAddr);
  fflush(stdout);
  return false;
}
...
void PatchSyncPrimitives() {
  ...
  PatchFuncMapping(
      0x829A9738,
      &sub_829A9738_hook); // Wait helper (NtWaitForSingleObjectEx) - CRITICAL
  PatchFuncMapping(0x829A3560,
                   &sub_829A3560_hook); // XamTask + mount integration (signals
                                        // completion event)
  PatchFuncMapping(0x829A39A0, &sub_829A39A0_hook); // Root sync primitive
  PatchFuncMapping(0x829A3238,
                   &sub_829A3238_hook); // Sync wait function
                                        // (KeWaitForSingleObject) - CRITICAL
  PatchFuncMapping(
      0x829A39F0,
      &sub_829A39F0_hook); // Intermediate sync (calls sub_829A3238) - CRITICAL
  PatchFuncMapping(0x827DB338, &sub_827DB338_hook); // Sync wait wrapper
  PatchFuncMapping(0x827DB988, &sub_827DB988_hook); // Sync init
  // sub_827DACD8 now uses direct PPC_FUNC override instead of PatchFuncMapping

  PatchFuncMapping(0x82300C78, &sub_82300C78_hook); // String/resource lookup
  PatchFuncMapping(0x829A2380,
                   &sub_829A2380_hook); // Semaphore acquire (sync table)
  PatchFuncMapping(0x829A21F8,
                   &sub_829A21F8_hook); // Semaphore create wrapper (sync table)
  PatchFuncMapping(
      0x829D8568,
...
struct SyncPrimitivePatcher {
  SyncPrimitivePatcher() { PatchSyncPrimitives(); }
};
...
// Forward declarations - actual tracking functions defined after Semaphore
// struct
static std::mutex g_semaphoreTrackMutex;
static std::unordered_set<uint32_t>
    g_blockingSemaphoreAddrs; // Guest addresses of blocking semaphores
...
bool IsInitComplete() { return g_initComplete.load(); }

// SetInitComplete and SignalAllBlockingSemaphores defined after Semaphore
// struct
...
struct Semaphore final : KernelObject, HostObject<XKSEMAPHORE> {
  ...
  uint32_t Wait(uint32_t timeout) override {
    ...
    if (timeout == 0) {
      ...
      // CRITICAL FIX: Yield to prevent spin-wait when polling
      // RenderThread polls with timeout=0 - without yield it burns CPU
      ...
      while (true) {
        ...
        if (currentCount != 0) {
          if (count.compare_exchange_weak(currentCount, currentCount - 1)) {
            if (s_waitCount <= 30)
              LOGF_WARNING("[SemWait] #{} acquired count={} after {} loops",
                           s_waitCount, currentCount, loopCount);
            return STATUS_SUCCESS;
          }
        } else {
          if (loopCount == 0 && s_waitCount <= 30)
            LOGF_WARNING("[SemWait] #{} blocking on count=0 (max={})...",
                         s_waitCount, maximumCount);
          ++loopCount;
          count.wait(0);
        }
      }
...
// Signals unconditionally to wake workers that may have acquired and are
// waiting again
void SignalAllBlockingSemaphores() {
  ...
  for (uint32_t addr : addrsCopy) {
    ...
    XKSEMAPHORE *semaphore =
        reinterpret_cast<XKSEMAPHORE *>(g_memory.Translate(addr));
...
void SetInitComplete() {
  if (!g_initComplete.exchange(true)) {
    LOGF_WARNING(
        "[SEMAPHORE_FIX] Init complete - signaling {} blocked semaphores",
...
bool TryWaitSemaphoreWithTimeout(uint32_t semAddr, uint32_t timeoutMs) {
  XKSEMAPHORE *semaphore =
      reinterpret_cast<XKSEMAPHORE *>(g_memory.Translate(semAddr));
  if (!semaphore || semaphore->Header.Type != 5)
    return false;
...
uint32_t NtClose(uint32_t handle) {
  ...
  if (IsKernelObject(handle)) {
    // If the handle was duplicated, just decrement the duplication count.
    // Otherwise, delete the object.
    auto it = g_handleDuplicates.find(handle);
    const bool willDestroy =
        (it == g_handleDuplicates.end() || it->second == 0);
    if (!willDestroy) {
      if (--it->second == 0)
...
uint32_t NtWaitForSingleObjectEx(uint32_t Handle, uint32_t WaitMode,
  ...
  // FIX: NULL handle (0x00000000) means "no object to wait on" - return success
  // immediately This unblocks the main thread after init completes when game
  // passes NULL handles
  if (Handle == 0) {
    static int s_nullWaitCount = 0;
    if (++s_nullWaitCount <= 5) {
      LOGF_WARNING("[NtWaitEx] NULL handle wait #{} - returning STATUS_SUCCESS "
                   "to unblock",
                   s_nullWaitCount);
    }
  ...
  if (s_waitCount <= 20 || s_waitCount % 500 == 0) {
    LOGF_IMPL(Utility, "NtWaitEx",
              "#{} handle=0x{:08X} timeout={} caller=0x{:08X}", s_waitCount,
              Handle, timeout, callerLR);
  }
  ...
  if (IsKernelObject(Handle)) {
    ...
    // 1. A kernel object handle (from NtCreateSemaphore/NtCreateEvent) -
    // directly a KernelObject*
    // 2. A guest dispatcher object address (XKSEMAPHORE/XKEVENT from
    // KeInitialize*) - needs QueryKernelObject

    // Check if this is a guest dispatcher object by looking at the header
    XDISPATCHER_HEADER *header =
        reinterpret_cast<XDISPATCHER_HEADER *>(g_memory.Translate(Handle));
    ...
    if (objType == 5) {
      ...
      if (s_waitCount <= 30 || (Handle >= 0xEB2D0000 && Handle < 0xEB2E0000)) {
        LOGF_WARNING("[NtWaitEx] #{} handle=0x{:08X} is guest XKSEMAPHORE, "
                     "queried kernel obj={}",
      ...
      if (s_waitCount <= 30) {
        LOGF_WARNING("[NtWaitEx] #{} handle=0x{:08X} is guest XKEVENT type={}, "
                     "queried kernel obj={}",
    ...
    if (!obj) {
      LOGF_WARNING(
          "[NtWaitEx] #{} handle=0x{:08X} type={} - no kernel object found!",
          s_waitCount, Handle, objType);
      return STATUS_TIMEOUT;
    }

    // KERNEL POLICY: Handle timeout=0 polling that goes on too long
    // Some game code busy-waits with timeout=0 polls on events that may never
    // be signaled After excessive polling, return SUCCESS to allow progress
    if (timeout == 0) {
      static int s_totalPolls = 0;
      ++s_totalPolls;

      // First try the actual wait
      uint32_t result = obj->Wait(0);
      if (result == STATUS_SUCCESS) {
        return result;
      }

      // If polling excessively overall, force success to break loops
      if (s_totalPolls > 50000 && s_totalPolls % 10000 == 0) {
        LOGF_WARNING("[NtWaitEx] POLL LIMIT: total polls={}, forcing SUCCESS "
                     "for handle=0x{:08X}",
                     s_totalPolls, Handle);
        return STATUS_SUCCESS;
      }
    ...
    if (timeout == INFINITE && ShouldFailOpenWait()) {
      ...
      if (result == STATUS_TIMEOUT) {
        ...
        if (s_failOpenObj <= 50 || s_failOpenObj % 1000 == 0) {
          LOGF_WARNING("[FAIL-OPEN] NtWaitEx #{} infinite->success "
                       "handle=0x{:08X} caller=0x{:08X}",
    ...
    // Removed worker semaphore bypass hack - let C++20 atomic wait/notify work
    // naturally Following Unleashed's approach: proper synchronization without
    // forced returns
    ...
    if (++s_unknownWaitCount <= 10) {
      LOGF_WARNING("[NtWaitEx] Unrecognized handle 0x{:08X} (occurrence #{})",
                   Handle, s_unknownWaitCount);
    }
  }
...
uint32_t NtCreateEvent(be<uint32_t> *handle, void *objAttributes,
  ...
  *handle = h;
  ...
  if (s_count <= 30) {
    LOGF_IMPL(Utility, "NtCreateEvent",
              "#{} CREATED handle=0x{:08X} type={} initial={}", s_count, h,
...
uint32_t NtReadFile(
  ...
  auto signalCompletionEvent = [&]() {
    if (Event != 0 && Event != GUEST_INVALID_HANDLE_VALUE &&
      ...
      XDISPATCHER_HEADER *eventObj =
          reinterpret_cast<XDISPATCHER_HEADER *>(g_memory.Translate(Event));
...
void VdSwap() {
  ...
  // Workers are waiting on events with infinite timeout - they need to be
  // signaled
  {
    std::lock_guard lock(g_eventTrackMutex);
    ...
    if (s_signalLogCount < 5) {
      ++s_signalLogCount;
      LOGF_UTILITY("VdSwap signaled {} events (gen={})",
                   g_trackedEventHandles.size(), g_keSetEventGeneration.load());
    }
  }
  ...
  static const std::vector<uint32_t> workerSemaphores = {
      0xA8240270, 0xA82402B0, 0xA82402F0, 0xA8240370, 0xA82403B0, 0xA82403F0,
      0xA8240430, 0xA8240470, 0xA82404B0, 0xA82404F0, 0xA8240530, 0xA8240570};
  ...
  if (s_semSignalCount < 5) {
    ++s_semSignalCount;
    LOGF_UTILITY("VdSwap signaled {} worker semaphores",
                 workerSemaphores.size());
  }
}
...
uint32_t VdRetrainEDRAM() {
  ...
  // GPU initialization complete - workers can proceed
  {
    ...
    if (s_callCount <= 3) {
      LOGF_UTILITY("VdRetrainEDRAM signaled {} events",
                   g_trackedEventHandles.size());
    }
  }
...
bool KeSetEvent(XKEVENT *pEvent, uint32_t Increment, bool Wait) {
  ...
  // Use TryQueryKernelObject first to avoid lock contention - object should
  // already exist
...
uint32_t KeWaitForSingleObject(XDISPATCHER_HEADER *Object, uint32_t WaitReason,
  ...
  const uint32_t objAddr =
      Object ? static_cast<uint32_t>(reinterpret_cast<uintptr_t>(Object) -
                                     reinterpret_cast<uintptr_t>(g_memory.base))
             : 0;

  // TRACK REPEATED WAITS: Find the handle that's blocking
  // Exclude GPU fence handles (0x8006F844, 0x8006F7F4) and GPU thread
  // (0x829DDD48) - already handled
  bool isGpuWait =
      (objAddr == 0x8006F844 || objAddr == 0x8006F7F4 || caller == 0x829DDD48);
  ...
  if (timeout == INFINITE && !isGpuWait) {
    ...
    if (s_infiniteNonGpu <= 30 || s_infiniteNonGpu % 100 == 0) {
      LOGF_WARNING("[INFINITE WAIT] #{} thread=0x{:04X} handle=0x{:08X} "
                   "type={} caller=0x{:08X}",
                   s_infiniteNonGpu, threadId, objAddr,
                   Object ? Object->Type : -1, caller);
    }
  }
  ...
  if (timeout == INFINITE && ShouldFailOpenWait() && !isGpuWait) {
    ...
    case 1:
      ...
      if (!obj)
        obj = QueryKernelObject<Event>(*Object);
      break;
    case 5:
      obj = TryQueryKernelObject<Semaphore>(*Object);
      if (!obj)
        obj = QueryKernelObject<Semaphore>(*Object);
      break;
    }
    ...
    if (obj) {
      ...
      if (result == STATUS_TIMEOUT) {
        // Still timed out - return success (do not force signal, just let it
        // proceed)
        static int s_failOpenCount = 0;
        ++s_failOpenCount;
        if (s_failOpenCount <= 50 || s_failOpenCount % 1000 == 0) {
          LOGF_WARNING("[FAIL-OPEN] #{} KeWait timeout->success obj=0x{:08X} "
                       "type={} caller=0x{:08X}",
  ...
  default:
    LOGF_IMPL(Utility, "KeWaitForSingleObject",
              "Unrecognized kernel object type: {}", Object->Type);
    return STATUS_TIMEOUT;
  }
...
// =============================================================================
// TLS UNIFICATION: Route KeTls* APIs through Guest Memory (r13 +
// X360_TLS_OFFSET)
// =============================================================================
...
// This caused state mismatches where game code and kernel APIs saw different
// TLS.
//
...
// Maximum TLS slots in guest memory (X360_TLS_SIZE = 0x100 = 256 bytes = 64
// uint32_t slots)
static constexpr uint32_t MAX_GUEST_TLS_SLOTS =
    X360_TLS_SIZE / sizeof(uint32_t);
...
uint32_t KeTlsGetValue(uint32_t dwTlsIndex) {
  ...
  uint32_t tlsAddr =
      ctx->r13.u32 + X360_TLS_OFFSET + (dwTlsIndex * sizeof(uint32_t));
  // Direct guest memory access (can't use PPC_LOAD_U32 macro - no 'base' in
  // scope)
...
uint32_t KeTlsSetValue(uint32_t dwTlsIndex, uint32_t lpTlsValue) {
  ...
  uint32_t tlsAddr =
      ctx->r13.u32 + X360_TLS_OFFSET + (dwTlsIndex * sizeof(uint32_t));
  // Direct guest memory access (can't use PPC_STORE_U32 macro - no 'base' in
  // scope)
  uint32_t *ptr = reinterpret_cast<uint32_t *>(g_memory.base + tlsAddr);
...
uint32_t KeTlsAlloc() {
  ...
  if (index >= MAX_GUEST_TLS_SLOTS) {
    ...
    if (!warned) {
      LOGF_WARNING("[TLS] Allocated slot {} exceeds guest TLS capacity ({}), "
                   "using host fallback",
...
uint32_t NtCreateSemaphore(be<uint32_t> *Handle,
  ...
  *Handle = GetKernelHandle(sem);
  ...
  if (InitialCount == 0) {
    ...
    LOGF_WARNING("[NtCreateSemaphore] #{} handle=0x{:08X} count=0 max={} - "
                 "TRACKED for signaling",
...
uint32_t NtReleaseSemaphore(uint32_t Handle, uint32_t ReleaseCount,
                            int32_t *PreviousCount) {
  static int s_count = 0;
  ++s_count;

  // FIX: Handle is a guest address that needs to be resolved via kernel object
  // system Previously this took Semaphore* directly which caused signals to go
  // to wrong objects
  if (!IsKernelObject(Handle)) {
    if (s_count <= 20 || s_count % 100 == 0) {
      LOGF_WARNING("[NtReleaseSemaphore] #{} Invalid handle 0x{:08X}", s_count,
                   Handle);
    }
    return STATUS_INVALID_HANDLE;
  }
  ...
  if (!obj) {
    LOGF_WARNING(
        "[NtReleaseSemaphore] #{} Null kernel object for handle 0x{:08X}",
        s_count, Handle);
    return STATUS_INVALID_HANDLE;
  }
  ...
  if (!sem) {
    ...
    if (evt) {
      if (s_count <= 30 || s_count % 100 == 0) {
        LOGF_WARNING("[NtReleaseSemaphore] #{} handle=0x{:08X} is EVENT, "
                     "signaling as event",
                     s_count, Handle);
      }
      evt->Set();
    ...
    LOGF_WARNING(
        "[NtReleaseSemaphore] #{} handle=0x{:08X} is not a semaphore or event",
        s_count, Handle);
  ...
  if (s_count <= 30 || isBlockingSemaphore || s_count % 100 == 0) {
    ...
    LOGF_WARNING(
        "[NtReleaseSemaphore] #{} handle=0x{:08X} release={} count={}/{}{}",
        s_count, Handle, ReleaseCount, curCount, sem->maximumCount,
        isBlockingSemaphore ? " *** BLOCKING SEM ***" : "");
  }

  // the game releases semaphore with 1 maximum number of releases more than
  // once
...
uint32_t ExCreateThread(be<uint32_t> *handle, uint32_t stackSize,
  ...
  // Many titles use an XAPI thread startup wrapper that takes (entry, arg) in
  // (r3, r4) and then performs an indirect call to entry.
  ...
  if (startContext != 0 && startContext >= 0x82000000 &&
      startContext < 0x84000000) {
    uint32_t *ctxMem =
        reinterpret_cast<uint32_t *>(g_memory.Translate(startContext));
    LOGF_IMPL(Utility, "ExCreateThread",
              "startContext@0x{:08X}: [0]=0x{:08X}, [1]=0x{:08X}, "
              "[2]=0x{:08X}, [3]=0x{:08X}",
              startContext, ByteSwap(ctxMem[0]), ByteSwap(ctxMem[1]),
              ByteSwap(ctxMem[2]), ByteSwap(ctxMem[3]));
  }
  ...
  if (creationFlags != 0) {
    LOGF_WARNING("[ExCreateThread] FORCING flags=0 (was 0x{:X}) entry=0x{:08X}",
                 creationFlags, entry);
    creationFlags = 0;
  }

  LOGF_WARNING("[ExCreateThread] Creating thread entry=0x{:08X} r3=0x{:08X} "
               "r4=0x{:08X} flags=0x{:X}",
               entry, r3, r4, creationFlags);
  *handle = GetKernelHandle(
      GuestThread::Start({entry, r3, r4, creationFlags}, &hostThreadId));
  LOGF_UTILITY("0x{:X}, 0x{:X}, 0x{:X}, 0x{:X}, 0x{:X}, 0x{:X}, 0x{:X} {:X}",
               (intptr_t)handle, stackSize, (intptr_t)threadId,
...
uint32_t KeWaitForMultipleObjects(uint32_t Count,
  ...
  if (s_callCount <= 20 || s_callCount % 100 == 0) {
    LOGF_WARNING("[KeWaitMultiple] #{} thread=0x{:04X} count={} waitType={} "
                 "caller=0x{:08X}",
  ...
  if (WaitType == 0) // Wait all
  {
    ...
    while (true) {
      ...
      if (loopCount <= 5 || loopCount % 1000 == 0) {
        LOGF_WARNING(
            "[KeWaitMultiple] Loop #{} thread=0x{:04X} waiting on {} objects",
...
uint32_t KeReleaseSemaphore(XKSEMAPHORE *semaphore, uint32_t increment,
  ...
  if (s_count <= 30 || (semAddr >= 0xEB2D0000 && semAddr <= 0xEB2E0000)) {
    LOGF_WARNING("[KeReleaseSemaphore] #{} sem=0x{:08X} adj={}", s_count,
                 semAddr, adjustment);
  }

  auto *object = QueryKernelObject<Semaphore>(semaphore->Header);
  if (!object) {
    LOGF_WARNING("[KeReleaseSemaphore] #{} sem=0x{:08X} QueryKernelObject "
                 "returned NULL!",
                 s_count, semAddr);
    return STATUS_INVALID_HANDLE;
  }
...
uint32_t KeResumeThread(GuestThreadHandle *object) {
  ...
  LOGF_WARNING("[RESUME] KeResumeThread #{} thread=0x{:08X}", s_count,
               reinterpret_cast<uintptr_t>(object) & 0xFFFFFFFF);
...
void KeInitializeSemaphore(XKSEMAPHORE *semaphore, uint32_t count,
  ...
  if (count == 0) {
    ...
    LOGF_WARNING("[KeInitializeSemaphore] #{} BLOCKING sem=0x{:08X} count=0 "
                 "limit={} - TRACKED",
                 s_count, semAddr, limit);
  } else if (s_count <= 30) {
    LOGF_WARNING(
        "[KeInitializeSemaphore] #{} sem=0x{:08X} count={} limit={} object={}",
...
uint32_t XamTaskSchedule(uint32_t funcAddr, uint32_t context,
  ...
  XDISPATCHER_HEADER *completionEvent = reinterpret_cast<XDISPATCHER_HEADER *>(
      g_memory.Translate(kTaskCompletionEventAddr));
  if (completionEvent &&
      (completionEvent->Type == 0 || completionEvent->Type == 1)) {
...
// Previously STUBBED: This function called an indirect task function that
// blocks forever FIX: Execute the task function ONCE synchronously, then return
// This allows resource loading to complete while avoiding the infinite
// semaphore-wait loop
//
...
PPC_FUNC(sub_827DAE40) {
  ...
  if (startupContext < 0x80000000 || startupContext >= 0x90000000) {
    LOGF_WARNING("[STREAMING_WORKER] sub_827DAE40 #{} INVALID context 0x{:08X} "
                 "- skipping",
  ...
  LOGF_WARNING("[STREAMING_WORKER] sub_827DAE40 #{} startupCtx=0x{:08X} "
               "taskFunc=0x{:08X} realCtx=0x{:08X}",
  ...
  if (taskFunc != 0 && taskFunc >= 0x82000000 && taskFunc < 0x83000000) {
    ...
    if (func) {
      LOGF_WARNING("[STREAMING_WORKER] sub_827DAE40 #{} EXECUTING taskFunc "
                   "0x{:08X} with realCtx=0x{:08X}",
                   s_count, taskFunc, realContext);

      // Execute task with REAL context (has semaphores), not startup context
      PPCContext taskCtx = ctx;
      taskCtx.r3.u32 =
          realContext; // FIX: Pass real context, not startup context
      func(taskCtx, base);

      LOGF_WARNING(
          "[STREAMING_WORKER] sub_827DAE40 #{} taskFunc 0x{:08X} COMPLETED",
          s_count, taskFunc);
    } else {
      LOGF_WARNING("[STREAMING_WORKER] sub_827DAE40 #{} taskFunc 0x{:08X} NOT "
                   "FOUND - skipping",
                   s_count, taskFunc);
    }
  } else {
    LOGF_WARNING("[STREAMING_WORKER] sub_827DAE40 #{} taskFunc 0x{:08X} OUT OF "
                 "RANGE - skipping",
                 s_count, taskFunc);
  }
...
//
// FIX: Detect garbage dimensions and skip the call entirely
extern "C" void __imp__sub_829E4970(PPCContext &ctx, uint8_t *base);
PPC_FUNC(sub_829E4970) {
  static int s_count = 0;
  ++s_count;
  ...
  uint32_t offsetStruct = ctx.r6.u32;
  uint32_t src = ctx.r7.u32;
  uint32_t bpp = ctx.r10.u32;

  // CRITICAL: Detect garbage parameters that would cause infinite loop
  // Valid texture dimensions: 1-8192, valid bpp: 1-16
  // Garbage indicators: dimensions > 16384, or dimensions look like addresses
  // (> 0x80000000)
  bool hasGarbageParams = false;

  if (width > 16384 || height > 16384) {
    hasGarbageParams = true;
  }
  ...
  // Log parameters (first few and periodically)
  if (s_count <= 5 || s_count % 500 == 0) {
    LOGF_WARNING(
        "[TEXTURE_TILE] #{} w={} h={} bpp={} dest=0x{:08X} src=0x{:08X}",
        s_count, width, height, bpp, dest, src);
  }

  if (hasGarbageParams) {
    if (++s_skipCount <= 10) {
      LOGF_WARNING("[TEXTURE_TILE] #{} SKIPPING - garbage params: w=0x{:08X} "
                   "h=0x{:08X} bpp={}",
                   s_count, width, height, bpp);
    }
    // Skip the call - don't process garbage dimensions
    return;
  }

  __imp__sub_829E4970(ctx, base);
}
...
// =============================================================================
// This function looks up entries in a structure. It crashes because:
// - r31+20 contains a pointer that's garbage (0x100000000+)
// - When added to base, this gives 0x400000000 causing EXC_BAD_ACCESS
//
// FIX: Validate pointers before dereferencing
extern "C" void __imp__sub_82859E98(PPCContext &ctx, uint8_t *base);
PPC_FUNC(sub_82859E98) {
  static int s_count = 0;
  ++s_count;

  uint32_t a1 = ctx.r3.u32; // Structure pointer
  uint32_t a2 = ctx.r4.u32; // Index/key

  // Early return if a2 is 0 (matches decompiled logic)
  if (a2 == 0) {
    ctx.r3.u32 = 0;
    return;
  }

  // Compute v5 = 48 * a2 + *(a1 + 8) - 48
  uint32_t basePtr = PPC_LOAD_U32(a1 + 8);
  uint32_t v5 = 48 * a2 + basePtr - 48;

  // Validate v5 is in a reasonable range
  if (v5 < 0x80000000 || v5 > 0xE0000000) {
    if (s_count <= 10) {
      LOGF_WARNING("[sub_82859E98] #{} INVALID v5=0x{:08X} (a1=0x{:08X} a2={} "
                   "basePtr=0x{:08X}) - returning 0",
                   s_count, v5, a1, a2, basePtr);
    }
    ctx.r3.u32 = 0;
    return;
  }

  // Check the pointer at v5+20
  uint32_t ptrAtV5_20 = PPC_LOAD_U32(v5 + 20);
  if (ptrAtV5_20 < 0x80000000 || ptrAtV5_20 > 0xE0000000) {
    if (s_count <= 10) {
      LOGF_WARNING("[sub_82859E98] #{} INVALID ptr at v5+20=0x{:08X} "
                   "(v5=0x{:08X}) - returning 0",
                   s_count, ptrAtV5_20, v5);
    }
    ctx.r3.u32 = 0;
    return;
  }

  // Pointers look valid, proceed with original function
  __imp__sub_82859E98(ctx, base);
}
...
// sub_827DACD8 - Semaphore wait with timeout fallback
// This wraps the original function but adds timeout to prevent infinite blocks
extern "C" void __imp__sub_827DACD8(PPCContext &ctx, uint8_t *base);
PPC_FUNC(sub_827DACD8) {
  ...
  // Null handle - return success immediately (matches original behavior)
  if (semHandle == 0) {
    ctx.r3.u32 = 1;
    return;
  }
  ...
  if (s_count <= 20 || s_count % 500 == 0) {
    printf("[sub_827DACD8] #%d sem=0x%08X\n", s_count, semHandle);
    fflush(stdout);
  }

  // Call original with a short internal timeout mechanism
  // The original function calls sub_829A2380 which we've hooked to return
...
PPC_FUNC(sub_829D0268) {
  ...
  LOGF_WARNING("[sub_82851F30] sub_829D0268 EXIT #{} r3={}", s_count,
               ctx.r3.s32);
}
...
PPC_FUNC(sub_8298E700) {
  ...
  if (ctx_ptr >= 0x82000000 && ctx_ptr < 0x90000000) {
    vtablePtr =
        ByteSwap(*(uint32_t *)g_memory.Translate(ctx_ptr + 0)); // vtable
    sem1 = ByteSwap(
        *(uint32_t *)g_memory.Translate(ctx_ptr + 36)); // wait semaphore
    sem2 = ByteSwap(
        *(uint32_t *)g_memory.Translate(ctx_ptr + 40)); // signal semaphore
  }

  LOGF_WARNING("[AUDIO_WORKER] sub_8298E700 #{} ctx=0x{:08X} vtable=0x{:08X} "
               "sem1=0x{:08X} sem2=0x{:08X}",
               s_count, ctx_ptr, vtablePtr, sem1, sem2);

  // Step 1: Signal sem2 so sub_8298E810 can continue
  if (sem2 != 0) {
    NtReleaseSemaphore(sem2, 1, nullptr);
    LOGF_WARNING("[AUDIO_WORKER] sub_8298E700 #{} signaled sem2=0x{:08X}",
                 s_count, sem2);
  }
  ...
  if (vtablePtr >= 0x82000000 && vtablePtr < 0x83000000) {
    uint32_t audioFunc =
        ByteSwap(*(uint32_t *)g_memory.Translate(vtablePtr + 12));
    if (audioFunc >= 0x82000000 && audioFunc < 0x83000000) {
      auto func = g_memory.FindFunction(audioFunc);
      if (func) {
        LOGF_WARNING(
            "[AUDIO_WORKER] sub_8298E700 #{} calling vtable[3]=0x{:08X}",
            s_count, audioFunc);
        PPCContext audioCtx = ctx;
        audioCtx.r3.u32 = ctx_ptr; // Pass context as 'this'
        func(audioCtx, base);
        LOGF_WARNING("[AUDIO_WORKER] sub_8298E700 #{} vtable[3] returned",
                     s_count);
      }
    }
  }
...
PPC_FUNC(sub_828935B8) {
  ...
  if (hostPtr) {
    ...
    if (allocSize >= 500000 && allocSize <= 600000) {
      ...
      printf("[sub_828935B8] SPEECH_DAT_BUFFER: 0x%08X size=%u\n", guestAddr,
             allocSize);
      fflush(stdout);
    }
...
// sub_8289DAA8 - Speechbanks data loader - NULL STREAM & SIZE VALIDATION FIX
// =============================================================================
// Called by sub_828A58C8 with a stream handle that may be NULL (when memory:
// URL fails) Also called recursively by sub_8289DD40 which reads 4 bytes and
// passes (value - 4) as size. When memory stream is exhausted, the read returns
// garbage/0 leading to huge/negative sizes.
//
// FIX: Check for NULL stream AND validate size to prevent:
//   1. Crash from NULL dereference
//   2. Infinite loop from huge sizes (e.g., -506 interpreted as 4294966790)
// =============================================================================
...
PPC_FUNC(sub_8289DAA8) {
  ...
  uint32_t a1 = ctx.r3.u32; // Audio manager context
  uint32_t a2 = ctx.r4.u32; // Stream handle (may be NULL)
  uint32_t a3 = ctx.r5.u32; // Size (may be invalid if stream exhausted)

  // NULL STREAM CHECK - prevent crash from memory: URL failure
  if (a2 == 0) {
    if (s_count <= 10) {
      LOGF_WARNING("[AUDIO_SPEECH] sub_8289DAA8 #{} NULL stream - skipping to "
                   "prevent crash",
                   s_count);
    }
    ctx.r3.u32 = 1; // Return success to continue (data already loaded)
    return;
  }

  // SIZE VALIDATION - prevent infinite loop from exhausted memory stream
  // When memory stream is exhausted, sub_8289DD40 reads garbage (0 or old
  // data), subtracts 4, and we get a huge/negative size like 0xFFFFFE06 (-506
  // as unsigned) Maximum reasonable speechbank size is ~10MB
  constexpr uint32_t MAX_SPEECHBANK_SIZE = 10 * 1024 * 1024; // 10MB
  if (a3 == 0 || a3 > MAX_SPEECHBANK_SIZE) {
    if (s_count <= 10) {
      LOGF_WARNING("[AUDIO_SPEECH] sub_8289DAA8 #{} INVALID size={} (0x{:08X}) "
                   "- returning success",
                   s_count, a3, a3);
    }
    ctx.r3.u32 =
        1; // Return success - speechbanks already loaded from earlier calls
    return;
  }

  if (s_count <= 10) {
    LOGF_WARNING("[AUDIO_SPEECH] sub_8289DAA8 #{} ENTER a1=0x{:08X} "
                 "stream=0x{:08X} size={}",
                 s_count, a1, a2, a3);
  }
  ...
  if (s_count <= 10) {
    LOGF_WARNING("[AUDIO_SPEECH] sub_8289DAA8 #{} EXIT r3={}", s_count,
                 ctx.r3.u32);
  }
}

// sub_828A58C8 - Speech Config Parser - UN-STUBBED (memory: URL now
// implemented)
// =============================================================================
// Parses speech.dat config and opens "speechbanks" via memory: URL.
// Memory: URL support implemented in sub_827E8180 to create memory streams.
// =============================================================================
...
PPC_FUNC(sub_828A58C8) {
  ...
  uint32_t a1 = ctx.r3.u32; // Input path string

  if (s_count <= 5) {
    LOGF_WARNING("[AUDIO_SPEECH] sub_828A58C8 #{} ENTER a1=0x{:08X}", s_count,
                 a1);
  }
  ...
  if (s_count <= 5) {
    LOGF_WARNING("[AUDIO_SPEECH] sub_828A58C8 #{} EXIT r3={}", s_count,
                 ctx.r3.u32);
  }
}
...
// Global flag to enable detailed read tracing during sub_8289D490
static thread_local bool g_traceAudioReads = false;
static thread_local int g_audioReadCount = 0;
static thread_local int g_audioConfigNum = 0;

extern "C" void __imp__sub_8289D490(PPCContext &ctx, uint8_t *base);
PPC_FUNC(sub_8289D490) {
  static int s_count = 0;
  ++s_count;
  g_audioConfigNum = s_count;

  uint32_t a1 = ctx.r3.u32;
  uint32_t a2 = ctx.r4.u32;

  // Read key structure fields from a1 (audio config structure)
  uint32_t basePtr = PPC_LOAD_U32(a1);        // *a1 = buffer base
  uint32_t bufferSize = PPC_LOAD_U32(a1 + 8); // *(a1+8) = buffer size
  uint32_t offset12 = PPC_LOAD_U32(a1 + 12);  // *(a1+12) = data offset
  uint32_t offset44 = PPC_LOAD_U32(a1 + 44);  // *(a1+44) = string buffer size
  uint32_t flag68 =
      PPC_LOAD_U8(a1 + 68); // *(a1+68) = flag for string processing (byte)

  if (s_count <= 5) {
    LOGF_WARNING(
        "[AUDIO_CONFIG] sub_8289D490 #{} ENTER a1=0x{:08X} a2=0x{:08X}",
        s_count, a1, a2);
    LOGF_WARNING(
        "[AUDIO_CONFIG]   base=0x{:08X} size={} offset={} strSize={} flag={}",
        basePtr, bufferSize, offset12, offset44, flag68);
  }

  // VALIDATION: Check if stream can provide valid data
  // The function reads offset values and uses them to compute addresses.
  // If stream position is wrong, offsets will be garbage (like ASCII data).
  // Validate by checking if the first offset value in stream looks reasonable.
  uint32_t streamDevice = PPC_LOAD_U32(a2 + 0);
  uint32_t streamPos = PPC_LOAD_U32(a2 + 12);

  // For small config files (< 10KB), parsing often fails due to stream position
  // issues Check if buffer size is small and stream position seems off
  if (bufferSize < 10000 && bufferSize > 0) {
    // Peek at stream data to validate format
    // Read the first 4 bytes that would be v55 (entry count)
    uint32_t peekAddr = PPC_LOAD_U32(a2 + 8);    // buffer address
    uint32_t peekCursor = PPC_LOAD_U32(a2 + 16); // buffer cursor
    uint32_t peekAvail = PPC_LOAD_U32(a2 + 20);  // bytes in buffer

    if (peekAvail >= 4 && peekAddr != 0) {
      uint32_t firstVal = PPC_LOAD_U32(peekAddr + peekCursor);
      // If first value looks like ASCII (high byte is printable char), stream
      // is misaligned
      uint8_t highByte = (firstVal >> 24) & 0xFF;
      if (highByte >= 0x20 && highByte <= 0x7E) {
        // Stream contains ASCII at offset position - parsing will fail
        static int s_skipCount = 0;
        if (++s_skipCount <= 3) {
          LOGF_WARNING("[AUDIO_CONFIG] sub_8289D490 #{} SKIP - stream "
                       "misaligned (first=0x{:08X} looks like ASCII)",
                       s_count, firstVal);
        }
        ctx.r3.u32 = 1; // Return success to continue loading
        return;
      }
    }
  }

  // Get file size from stream structure to validate loop counts
  uint32_t streamFileSize = PPC_LOAD_U32(a2 + 28); // fileSize at offset 28
  uint32_t streamFilePos = PPC_LOAD_U32(a2 + 12);
  uint32_t streamBufCursor = PPC_LOAD_U32(a2 + 16);
  uint32_t streamBytesInBuf = PPC_LOAD_U32(a2 + 20);
  uint32_t currentEffPos = streamFilePos - streamBytesInBuf + streamBufCursor;
  uint32_t remainingBytes =
      (streamFileSize > currentEffPos) ? (streamFileSize - currentEffPos) : 0;

  // CRITICAL FIX: For small audio config files, the second loop count (v54)
  // sometimes reads garbage (string data) causing millions of iterations and
  // EOF overrun. The file categories.dat15 has v55=0 (first loop skipped), then
  // position 8138 contains string data ".BAS" (0x04424153) which is interpreted
  // as v54 = 71 million.
  //
  // Solution: If buffer size is small and remaining bytes are limited,
  // we know the file can't support millions of loop iterations.
  // Cap the effective loop count by validating after the original function call
  // would fail.
  //
  // Actually, we need to prevent the crash entirely. Since we can't easily
  // modify the loop inside the original function, we check if the file format
  // looks wrong and skip processing if it does.

  // For small config files, peek ahead to check if second v54 position has
  // valid data
  if (bufferSize < 15000 && remainingBytes < 10000) {
    // After v55, a1+56, and first v54 (12 bytes), the second v54 would be at
    // +12 from current If that position has a value > remaining bytes / 4, it's
    // garbage
    uint32_t peekBufAddr = PPC_LOAD_U32(a2 + 8);
    uint32_t peekBufCursor = PPC_LOAD_U32(a2 + 16);
    uint32_t peekBytesInBuf = PPC_LOAD_U32(a2 + 20);

    // Peek at position +12 (after v55, a1+56, first v54)
    if (peekBytesInBuf >= peekBufCursor + 16 && peekBufAddr != 0) {
      uint32_t secondV54Pos = peekBufAddr + peekBufCursor + 12;
      uint32_t secondV54Val = PPC_LOAD_U32(secondV54Pos);

      // If second v54 > 10000 for a small file, it's likely garbage string data
      if (secondV54Val > 10000) {
        static int s_skipCount = 0;
        if (++s_skipCount <= 3) {
          LOGF_WARNING("[AUDIO_CONFIG] sub_8289D490 #{} SKIP - second loop "
                       "count 0x{:08X} too large (remaining={})",
                       s_count, secondV54Val, remainingBytes);
        }
        ctx.r3.u32 = 1; // Return success to continue loading
        return;
      }
  ...
  g_traceAudioReads =
      false; // Disable verbose tracing now that we understand the issue
  g_audioReadCount = 0;

  __imp__sub_8289D490(ctx, base);

  g_traceAudioReads = false;

  if (s_count <= 5) {
    LOGF_WARNING("[AUDIO_CONFIG] sub_8289D490 #{} EXIT r3={} (reads={})",
                 s_count, ctx.r3.u32, g_audioReadCount);
  }
}
...
extern "C" void sub_829A3238_hook(PPCContext &ctx, uint8_t *base) {
  ...
  XDISPATCHER_HEADER *syncEvent = reinterpret_cast<XDISPATCHER_HEADER *>(
      g_memory.Translate(kSyncEventAddr));
  if (syncEvent && (syncEvent->Type == 0 || syncEvent->Type == 1)) {
    ...
    printf("[SYNC] sub_829A3238 #%d Event at 0x%08X not ready (type=%d) - "
           "initializing\n",
...
extern "C" void sub_829A39F0_hook(PPCContext &ctx, uint8_t *base) {
  ...
  if (syncEvent && (syncEvent->Type == 0 || syncEvent->Type == 1)) {
    ...
    printf("[SYNC] sub_829A39F0 #%d Event at 0x%08X not ready (type=%d) - "
           "initializing as signaled\n",
           s_count, kSyncEventAddr, syncEvent ? syncEvent->Type : -1);
    // Initialize as a notification event in signaled state so sub_829A3238's
...
extern "C" void sub_829A3560_hook(PPCContext &ctx, uint8_t *base) {
  ...
  if (completionEvent &&
    ...
    printf("[SYNC] sub_829A3560 #%d Event at 0x%08X not ready (type=%d)\n",
           ...
           completionEvent ? completionEvent->Type : -1);
  }
  fflush(stdout);
...
extern "C" void sub_827DB988_hook(PPCContext &ctx, uint8_t *base) {
  ...
  if (syncEvent && (syncEvent->Type == 0 || syncEvent->Type == 1)) {
    ...
    printf("[SYNC] sub_827DB988 #%d Event at 0x%08X not ready (type=%d) - "
           "initializing as signaled\n",
           s_count, kSyncEventAddr, syncEvent ? syncEvent->Type : -1);
...
PPC_FUNC(sub_8218BEB0) {
  ...
  // NOTE: SetInitComplete is now called at sub_821200D0 ENTRY (in
  // save_hooks.cpp) This is the right timing: after 63-subsystem init, before
  // post-init loading
...
PPC_FUNC(sub_827E8420) {
  ...
  if (storageDevice == MEMORY_STREAM_MARKER) {
    ...
    // Structure: [0]=marker, [4]=memOffset, [8]=unused, [12]=position,
    // [16]=size, [20]=bytesRead
    uint32_t memOffset = fileHandle; // Relative offset within speech.dat buffer
    uint32_t position = filePos;     // Current read position
    uint32_t totalSize = bufferCursor; // Total size stored in slot 16

    // The memOffset is RELATIVE to the speech.dat buffer base
    // Use g_speechDatBufferBase to compute absolute address
    uint32_t absoluteAddr = g_speechDatBufferBase + memOffset;
    ...
    if (toRead > 0 && destAddr != 0 && g_speechDatBufferBase != 0) {
      ...
      if (s_memReadCount <= 10) {
        LOGF_WARNING("[sub_827E8420] MEMORY STREAM #{} base=0x{:08X} offset={} "
                     "pos={} read={}",
                     s_memReadCount, g_speechDatBufferBase, memOffset, position,
                     toRead);
      }
    } else if (s_memReadCount <= 10) {
      LOGF_WARNING("[sub_827E8420] MEMORY STREAM #{} SKIP - base=0x{:08X} "
                   "offset={} available={}",
                   s_memReadCount, g_speechDatBufferBase, memOffset, available);
    }
...
PPC_FUNC(sub_82124490) {
  ...
  if (ctx.r3.u32 != 0 && !s_gateCleared && s_count > 500) {
    ...
    s_gateCleared = true;
    LOGF_WARNING(
        "[LOADING_GATE] BYPASS: Forcing gate clear after {} iterations "
        "(Xbox threading/audio callbacks not emulated)",
        s_count);
  }
  ...
  if (s_count <= 20 || s_count % 1000 == 0) {
    ...
    uint8_t byte_BC9 = PPC_LOAD_U8(0x83137BC9);
    LOGF_WARNING("[LOADING_GATE] #{} BB7={} BC9={} r3={}", s_count, byte_BB7,
                 byte_BC9, ctx.r3.u32);
  }
}
...
// SIMPLIFIED APPROACH: Call original recompiled function, then ensure async
// completion Parameters: r3=handle, r4=buffer(guest), r5=size, r6=offset,
// r7=asyncInfo
PPC_FUNC(sub_829A1F00) {
  ...
  // ReadFromBestRpf has fallback logic to try all RPF files even without a
  // handle
  if (handle == 0) {
    static int s_nullReadCount = 0;
    if (++s_nullReadCount <= 10) {
      LOGF_WARNING("[GTA4_FileLoad] NULL handle read: offset=0x{:X} "
                   "size=0x{:X} (will try all RPFs)",
                   offset, size);
    }
  ...
  if (count <= 50 || count % 100 == 0) {
    LOGF_IMPL(Utility, "GTA4_FileLoad",
              "sub_829A1F00 #{} handle=0x{:08X} size=0x{:X} offset=0x{:X} "
              "async=0x{:08X}",
  ...
  const uint32_t bytesRead =
      ReadFromBestRpf(handle, hostBuffer, size, offset, count, chosen);

  if (bytesRead > 0) {
    g_handleToRpf[handle] = chosen;
    if (count <= 50 || count % 100 == 0) {
      LOGF_IMPL(Utility, "GTA4_FileLoad",
                "sub_829A1F00 #{} read {} bytes from '{}' at offset 0x{:X}",
                count, bytesRead, chosen, offset);
    }

    // For async reads, set completion flags BEFORE returning (static recomp
    // playbook) Xbox 360 IO_STATUS_BLOCK / OVERLAPPED structure (big-endian):
    //   Offset 0: Status (NTSTATUS) - 0 = STATUS_SUCCESS, 0x103 =
    //   STATUS_PENDING Offset 4: Information (bytes transferred)
    // The guest is POLLING this memory address waiting for Status !=
    // STATUS_PENDING
    if (asyncInfo != 0 && asyncInfo < 0x20000000) {
      volatile uint32_t *asyncPtr =
          reinterpret_cast<volatile uint32_t *>(base + asyncInfo);
      ...
      asyncPtr[1] =
          ByteSwap(bytesRead); // Information = bytes transferred (offset 4)
      std::atomic_thread_fence(std::memory_order_seq_cst);
      asyncPtr[0] = 0; // Status = STATUS_SUCCESS (0) - write LAST!
      std::atomic_thread_fence(std::memory_order_seq_cst);

      // Signal event at offset 8 if present (Xbox kernel would do this on I/O
      // completion) asyncInfo structure: +0=Status, +4=Information, +8=hEvent,
      // +12=?, +16=Flags
      uint32_t eventHandle = ByteSwap(asyncPtr[2]);
      if (eventHandle != 0 && eventHandle != 0xFFFFFFFF &&
          IsKernelObject(eventHandle)) {
        XDISPATCHER_HEADER *eventObj = reinterpret_cast<XDISPATCHER_HEADER *>(
            g_memory.Translate(eventHandle));
        if (eventObj && (eventObj->Type == 0 || eventObj->Type == 1)) {
          QueryKernelObject<Event>(*eventObj)->Set();
          if (count <= 20) {
            LOGF_IMPL(Utility, "GTA4_FileLoad",
                      "ASYNC: Signaled event 0x{:08X} for async completion",
                      eventHandle);
          }
        }
      }
      ...
      if (count <= 20) {
        ...
        LOGF_IMPL(Utility, "GTA4_FileLoad",
                  "ASYNC: AFTER  @0x{:08X}: Status=0x{:08X}, Info=0x{:08X}, "
                  "hEvent=0x{:08X}",
    ...
    if (!hFile->stream.bad()) {
      ...
      if (hFile->isRpf && hFile->tocEncrypted && bytesRead > 0) {
        ...
        if (overlapEnd > overlapStart) {
          ...
          if (count <= 20 || count % 100 == 0) {
            LOGF_IMPL(
                Utility, "GTA4_FileLoad",
                "Decrypting TOC region: bufOffset={} len={} tocRelOffset={}",
                startInBuf, len, tocRelativeOffset);
          }

          DecryptRpfBufferInPlace(hostBuffer + startInBuf, len,
                                  tocRelativeOffset);
        }
      }
  ...
  if (vit != g_ntVirtFileHandles.end() && vit->second &&
      vit->second->magic == kNtVirtFileHandleMagic) {
    NtVirtFileHandle *hVirt = vit->second;
    ...
    if (offset < hVirt->data.size()) {
      ...
      if (count <= 20 || count % 100 == 0) {
        LOGF_IMPL(
            Utility, "GTA4_FileLoad",
            "sub_829A1F00 #{} read {} bytes from virtual data at offset 0x{:X}",
            count, toCopy, offset);
      }
  ...
  if (count <= 20 || count % 100 == 0) {
    LOGF_IMPL(Utility, "GTA4_FileLoad",
              "sub_829A1F00 #{} FAILED for handle 0x{:08X}", count, handle);
  }
  ctx.r3.u32 = 0;
}

// NOTE: sub_827F0B20 is the retry loop - we let it run but our file system
// should handle reads properly The key is to ensure sub_829A1F00 returns
// correct data and completion flags
...
GUEST_FUNCTION_HOOK(__imp__XamUserCreateAchievementEnumerator,
                    XamUserCreateAchievementEnumerator);
GUEST_FUNCTION_HOOK(__imp__XeKeysConsoleSignatureVerification,
...
GUEST_FUNCTION_HOOK(__imp__XamUserCreateStatsEnumerator,
...
GUEST_FUNCTION_HOOK(__imp__XamUserWriteProfileSettings,
...
GUEST_FUNCTION_HOOK(__imp__XeKeysConsolePrivateKeySign,
                    XeKeysConsolePrivateKeySign);
GUEST_FUNCTION_HOOK(__imp__IoDismountVolumeByFileHandle,
                    IoDismountVolumeByFileHandle);
GUEST_FUNCTION_HOOK(__imp__NetDll_XNetGetEthernetLinkStatus,
                    Net::XNetGetEthernetLinkStatus);
GUEST_FUNCTION_HOOK(__imp__KeTryToAcquireSpinLockAtRaisedIrql,
                    KeTryToAcquireSpinLockAtRaisedIrql);
GUEST_FUNCTION_HOOK(__imp__XamShowGamerCardUIForXUID,
...
GUEST_FUNCTION_HOOK(__imp__XamLoaderGetLaunchDataSize,
...
GUEST_FUNCTION_HOOK(__imp__XamCreateEnumeratorHandle,
...
// We must signal all semaphores before the loop to ensure workers wake up and
// exit.
...
PPC_FUNC(sub_827DB880) {
  ...
  ++s_count;
  printf(
      "[827DB880] #%d ENTER - worker shutdown loop, signaling all semaphores\n",
      s_count);
  fflush(stdout);
