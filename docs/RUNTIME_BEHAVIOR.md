...
## 4. Kernel API Behavior Requirements
...
|-------|------|-------------|
| Object | XDISPATCHER_HEADER* | Event/Semaphore/Mutex |
| WaitReason | uint32_t | Reason code (informational) |
| WaitMode | uint32_t | Kernel/User mode |
| Alertable | bool | Allow APC interruption |
| Timeout | int64_t* | NULL=infinite, negative=relative 100ns |

| Return | Meaning |
|--------|---------|
| 0 | STATUS_SUCCESS / STATUS_WAIT_0 |
| 0x102 | STATUS_TIMEOUT |
| 258 | GPU fence retry (special) |
| 257 | Retry trigger for `sub_829A9738` |

**`NtWaitForSingleObjectEx`**:
Same as above but takes handle instead of object pointer. Used by async I/O wrappers.

### Event Functions

**`KeSetEvent`**:
- Sets event to signaled state
- Wakes all waiters (notification event) or one waiter (synchronization event)
- Returns previous state (TRUE if already signaled)
...
- `initialState`: 0 = non-signaled, 1 = signaled

### Semaphore Functions

**`KeReleaseSemaphore`**:
- Increments semaphore count by `adjustment`
- Wakes waiting threads if count becomes > 0
- Returns previous count

**`KeInitializeSemaphore`**:
- `initialCount`: Starting count (0 = immediately blocking)
- `maximumCount`: Maximum allowed count

### Critical Implementation Notes

1. **Guest dispatcher objects** (XKEVENT, XKSEMAPHORE) are at guest memory addresses
2. Host must use `QueryKernelObject<T>(*header)` to get backing kernel object
3. Direct handle values may be either:
   - Kernel object handles (from NtCreate*)
   - Guest dispatcher object addresses (from KeInitialize*)
4. Type field in XDISPATCHER_HEADER: 0/1 = Event, 5 = Semaphore

---

## 5. Cooperative Polling Patterns

### Pattern 1: Return Code 996 (No Progress)
```cpp
// Caller pattern - repeats until helper returns non-996
status = sub_829A1A50(obj+8, &out, 0);
if (status == 996) {
    return 0;  // No progress, try again later
}
// Continue with result
```

**Meaning**: Async operation not ready. Caller should retry.

### Pattern 2: Return Code 997 (Pending)
```cpp
// XamContentClose example
status = sub_829A1CA0(handle);
if (status == 997) {
    *(obj+4) = 4;  // Set sub-state to pending
    return 0;      // Return without cleanup
}
// Cleanup complete
```

**Meaning**: Operation still in progress. Store pending state and return.

### Pattern 3: Return Code 259 (Wait Required)
```cpp
// sub_829A1F00 - enforced wait barrier
...
**Meaning**: Must perform synchronous wait before continuing.

### Pattern 4: Tight Retry Loop
```cpp
// sub_827F0B20 - spins until non-zero return
countdown = 10;
do {
    status = sub_829A1F00(...);
    if (status == 0) {
        sub_829A1EF8();  // yield-like call
        --countdown;
        if (countdown < 0) {
            call_indirect_callback();
        }
    }
} while (status == 0);
```

**Warning**: This can spin indefinitely if `sub_829A1F00` always returns 0.

---

## 6. Boot-Critical Function Reference

| Function | Address | Role | Blocking Condition |
|----------|---------|------|-------------------|
| `sub_82120000` | 0x82120000 | One-time init | Returns 0 until ready |
| `sub_8218C600` | 0x8218C600 | Worker creation | Waits for XAM tasks |
| `sub_82673718` | 0x82673718 | Audio init | Polls job counters |
| `sub_829748D0` | 0x829748D0 | Worker init | Depends on main thread |
| `sub_8298E810` | 0x8298E810 | Resource init | Async completion |
| `sub_829A3318` | 0x829A3318 | Task orchestrator | XamTaskShouldExit loop |
| `sub_829DDC90` | 0x829DDC90 | GPU fence wait | Returns 258 to retry |
| `sub_82169400` | 0x82169400 | Global flag wait | Checks *(global+300) |
| `sub_821694C8` | 0x821694C8 | Global flag wait | Loops while !=0 |
| `sub_829A1F00` | 0x829A1F00 | Async driver | Returns 259 for wait |
| `sub_829A1A50` | 0x829A1A50 | Status helper | Returns 996 for retry |
...
- **Cooperative polling** uses magic return values (996, 997, 258, 259) that must be handled correctly.
...
- 2025-12-20: Added kernel API behavior requirements, cooperative polling patterns, and boot-critical function reference.
