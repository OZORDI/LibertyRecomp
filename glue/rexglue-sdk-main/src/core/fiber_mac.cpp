/**
 * @file        core/fiber_mac.cpp
 * @brief       macOS backend for rex::thread::Fiber (setjmp/longjmp + stack pivot)
 *
  ...
  if (setjmp(FIBER_JMP_BUF(from)) == 0) {
    if (target->started_) {
      ...
      __asm__ volatile(
          ...
          : "memory");
#else
      // Intel Mac (x86_64) — direct call grows the thread stack. This works
      // correctly on x86_64 macOS unlike ucontext on arm64.
      (void)sp;
      trampoline();
#endif
