...
static_assert(REX_PLATFORM_PS4, "This file is PS4-only");
...
uint32_t current_thread_system_id() {
  // PS4 (FreeBSD kernel): scePthreadGetthreadid() is the official TID.
...
class PosixCondition<Thread> : public PosixConditionBase {
 ...
 public:
  ...
  uint64_t affinity_mask() {
    // PS4: scePthreadGetaffinity(thread, uint64_t* mask) — bit i = allowed on CPU i.
    WaitStarted();
    uint64_t mask = 0;
  ...
  void QueueUserCallback(std::function<void()> callback) {
    ...
    // PS4 (FreeBSD-libc): no pthread_sigqueue; use plain pthread_kill. Payload
    // (sigval) isn't used by our handler.
    (void)value;
