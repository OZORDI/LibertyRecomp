...
async function main() {
    const server = new Server({ name: 'math-tools', version: '1.1.0' }, { capabilities: { tools: {} } });
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        try {
            ...
            switch (name) {
                // Hex/Address Math
                case 'hex_to_dec':
                    result = mathTools.hexToDec(args?.hex, args?.signed ?? true, args?.bits ?? 32);
                    break;
                case 'dec_to_hex':
                    result = mathTools.decToHex(args?.decimal, args?.bits ?? 32);
                    break;
                case 'add_offset':
                    result = mathTools.addOffset(args?.base, args?.offset);
                    break;
                case 'address_range':
                    result = mathTools.addressRange(args?.start, args?.size);
                    break;
                // PPC-Specific Math
                case 'lis_calculation':
                    result = mathTools.lisCalculation(args?.immediate);
                    break;
                case 'effective_address':
                    result = mathTools.effectiveAddress(args?.lis_value, args?.offset);
                    break;
                case 'ppc_offset_decode':
                    result = mathTools.ppcOffsetDecode(args?.value);
                    break;
                // Signed/Unsigned Conversions
                case 'twos_complement':
                    result = mathTools.twosComplement(args?.value, args?.bits ?? 32);
                    break;
                case 'sign_extend':
                    result = mathTools.signExtend(args?.value, args?.from_bits, args?.to_bits);
                    break;
                // Bit Operations
                case 'bit_mask':
                    result = mathTools.bitMask(args?.start_bit, args?.end_bit, args?.bits ?? 32);
                    break;
                case 'bit_shift':
                    result = mathTools.bitShift(args?.value, args?.amount, args?.direction, args?.logical ?? true, args?.bits ?? 32);
                    break;
                case 'bit_extract':
                    result = mathTools.bitExtract(args?.value, args?.start_bit, args?.num_bits);
                    break;
                case 'rlwinm':
                    result = mathTools.rlwinm(args?.value, args?.shift, args?.mask_begin, args?.mask_end);
                    break;
                // Timing Math
                case 'hz_to_ms':
                    result = mathTools.hzToMs(args?.hz);
                    break;
                case 'fps_calculator':
                    result = mathTools.fpsCalculator(args?.input, args?.input_type);
                    break;
                case 'timing_analysis':
                    result = mathTools.timingAnalysis(args?.target_fps, args?.actual_frame_time_ms);
                    break;
                // Struct/Memory Layout
                case 'struct_size':
                    result = mathTools.structSize(args?.offsets);
                    break;
                case 'align_address':
                    result = mathTools.alignAddress(args?.address, args?.alignment);
                    break;
                case 'page_align':
                    result = mathTools.pageAlign(args?.size, args?.page_size ?? 4096);
                    break;
                // General Arithmetic
                case 'calculate':
                    result = mathTools.calculate(args?.expression);
                    break;
                case 'base_convert':
                    result = mathTools.baseConvert(args?.value, args?.from_base, args?.to_base);
                    break;
                // ==================== NEW TOOL HANDLERS ====================
                // Memory Region Analysis
                case 'ppc_memory_map':
                    result = mathTools.ppcMemoryMap(args?.address);
                    break;
                case 'round_to_page':
                    result = mathTools.roundToPage(args?.size, args?.page_size ?? 4096);
                    break;
                case 'is_valid_ppc_address':
                    result = mathTools.isValidPpcAddress(args?.address);
                    break;
                // Xbox 360 Performance Counter
                case 'perf_ticks_to_ms':
                    result = mathTools.perfTicksToMs(args?.ticks);
                    break;
                case 'ms_to_perf_ticks':
                    result = mathTools.msToPerfTicks(args?.ms);
                    break;
                case 'timebase_to_seconds':
                    result = mathTools.timebaseToSeconds(args?.timebase);
                    break;
                // Byte Swapping / Endianness
                case 'byte_swap_16':
                    result = mathTools.byteSwap16(args?.value);
                    break;
                case 'byte_swap_32':
                    result = mathTools.byteSwap32(args?.value);
                    break;
                case 'byte_swap_64':
                    result = mathTools.byteSwap64(args?.value);
                    break;
                case 'byte_swap_float':
                    result = mathTools.byteSwapFloat(args?.value);
                    break;
                // NTSTATUS / Error Code Analysis
                case 'ntstatus_decode':
                    result = mathTools.ntstatusDecode(args?.status);
                    break;
                case 'ntstatus_is_error':
                    result = mathTools.ntstatusIsError(args?.status);
                    break;
                case 'ntstatus_is_warning':
                    result = mathTools.ntstatusIsWarning(args?.status);
                    break;
                // Allocation / Pool Math
                case 'allocation_units':
                    result = mathTools.allocationUnits(args?.bytes, args?.sectors_per_unit ?? 8, args?.bytes_per_sector ?? 512);
                    break;
                case 'sectors_to_bytes':
                    result = mathTools.sectorsToBytes(args?.sectors, args?.bytes_per_sector ?? 512);
                    break;
                default:
