...
export class MathTools {
    ...
    effectiveAddress(lisValue, offset) {
    ...
    signExtend(value, fromBits, toBits) {
        const val = BigInt(value.startsWith('0x') ? value : value);
    ...
    // ==================== BIT OPERATIONS ====================
    bitMask(startBit, endBit, bits = 32) {
    ...
    bitShift(value, amount, direction, logical = true, bits = 32) {
    ...
    bitExtract(value, startBit, numBits) {
        const val = BigInt(value.startsWith('0x') ? value : value);
    ...
    rlwinm(value, shift, maskBegin, maskEnd) {
    ...
    timingAnalysis(targetFps, actualFrameTimeMs) {
    ...
    roundToPage(size, pageSize = 4096) {
        const sizeVal = BigInt(size.startsWith('0x') ? size : size);
        const pageSizeBigInt = BigInt(pageSize);
    ...
    allocationUnits(bytes, sectorsPerUnit = 8, bytesPerSector = 512) {
    ...
    sectorsToBytes(sectors, bytesPerSector = 512) {
