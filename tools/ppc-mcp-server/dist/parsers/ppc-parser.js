...
export class PPCParser {
    ppcDir;
    importsFile;
    constructor(ppcDir, importsFile) {
        this.ppcDir = ppcDir;
    ...
    async parseFunctionMapping(index) {
    ...
    async parsePPCFile(filePath, index) {
    ...
    parseMemoryAccess(line, func, index, type) {
    ...
    trackComputedGlobals(lines, startLine, endLine, func, index) {
        ...
        for (let i = startLine; i < endLine && i < lines.length; i++) {
            ...
            if (loadMatch) {
                ...
                if (baseVal !== undefined) {
                    ...
                    if (addr.startsWith('82') || addr.startsWith('83')) {
                        this.addGlobalAccess(func, index, '0x' + addr, 'read', parseInt(size), line);
                    }
                }
            }
            ...
            if (storeMatch) {
                ...
                if (baseVal !== undefined) {
                    ...
                    const addr = ((baseVal + offset) >>> 0).toString(16).padStart(8, '0');
                    if (addr.startsWith('82') || addr.startsWith('83')) {
                        this.addGlobalAccess(func, index, '0x' + addr, 'write', parseInt(size), line);
                    }
                }
            }
        }
    }
    addGlobalAccess(func, index, address, type, size, context) {
...
export async function buildIndex(ppcDir, importsFile) {
