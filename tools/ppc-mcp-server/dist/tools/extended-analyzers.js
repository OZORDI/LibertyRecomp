...
export class ExtendedAnalyzer {
    ...
    findDeadCode() {
        ...
        for (const [, func] of this.index.functions) {
            ...
            if (func.name.startsWith('__imp__'))
                continue;
            if (this.index.hooks.has(func.name))
                continue;
    ...
    annotateFunction(funcName, notes, labels) {
    ...
    labelGlobal(address, name, type, notes) {
        const annotations = this.loadAnnotations();
    ...
    buildImportDependencyGraph() {
        ...
        for (const [, api] of this.index.kernelAPIs) {
            if (!categories[api.category])
                categories[api.category] = new Set();
    ...
    analyzeExecutionOrder(rootFunc) {
    ...
    visualizeStateMachine(funcName) {
        const func = this.index.functionsByName.get(funcName);
        if (!func)
            return null;
    ...
    traceRegisterFlow(funcName, register) {
        const func = this.index.functionsByName.get(funcName);
        if (!func || !func.file)
            return null;
        const filePath = path.join(this.ppcDir, func.file);
