// Rendering Analysis & Build Tools for PPC Recompilation MCP Server
...
const GPU_FUNCTIONS: Record<string, { category: string; description: string; params?: string }> = {
  // Draw calls
...
interface BuildConfig {
  platform: 'windows-x64' | 'windows-arm64' | 'linux-x64' | 'linux-arm64' | 'macos-arm64' | 'macos-x64';
  mode: 'debug' | 'relwithdebinfo' | 'release';
}

const BUILD_PRESETS: Record<string, { preset: string; buildDir: string }> = {
...
export class RenderingTools {
  ...
  constructor(private index: PPCIndex, private ppcDir: string) {
    this.projectRoot = path.resolve(ppcDir, '../..');
  }

  // ========== RENDER PATH TRACER ==========
  traceRenderPath(): {
    renderPath: { function: string; description: string; file?: string; hooked: boolean }[];
    initChain: { function: string; description: string; blocking?: string }[];
    gpuFunctions: { function: string; category: string; description: string; calledBy: string[] }[];
  } {
    const renderPath: { function: string; description: string; file?: string; hooked: boolean }[] = [];
    ...
    // Init chain
    const initChain: { function: string; description: string; blocking?: string }[] = [
    ...
    // GPU functions usage
    const gpuFunctions: { function: string; category: string; description: string; calledBy: string[] }[] = [];
  ...
  analyzeVdSwap(): {
    ...
    hookStatus: { isHooked: boolean; hookType?: string };
    callers: string[];
    events: string[];
    frameFlow: { step: string; description: string }[];
    knownIssues: string[];
  } {
  ...
  inspectGpuCommands(category?: string): {
    categories: string[];
    commands: { function: string; category: string; description: string; params?: string; hookStatus: string }[];
    ...
    const commands: { function: string; category: string; description: string; params?: string; hookStatus: string }[] = [];
    const drawCallFunctions: string[] = [];
    const resourceFunctions: string[] = [];
    const stateFunctions: string[] = [];
    ...
    for (const [funcName, info] of Object.entries(GPU_FUNCTIONS)) {
      ...
      if (info.category === 'draw') drawCallFunctions.push(funcName);
  ...
  trackRenderState(funcName: string): {
    function: string;
    stateAccess: { type: string; offset: number; description: string }[];
    deviceOffsets: { offset: number; usage: string }[];
    ...
    const deviceOffsets: { offset: number; usage: string }[] = [
    ...
    const renderTargetOps = func.kernelCalls.filter(k => 
      k.includes('RenderTarget') || k.includes('DepthStencil') || k.includes('Surface')
    );
    
    const shaderOps = func.kernelCalls.filter(k =>
      k.includes('Shader') || k.includes('shader')
    );
  ...
  analyzeShaderUsage(): {
    vertexShaderFunctions: { name: string; callers: number }[];
    pixelShaderFunctions: { name: string; callers: number }[];
    shaderCreation: { function: string; type: string }[];
    shaderBinding: { function: string; description: string }[];
  } {
    const vsSetters: { name: string; callers: number }[] = [];
    const psSetters: { name: string; callers: number }[] = [];
    const shaderCreation: { function: string; type: string }[] = [];
    const shaderBinding: { function: string; description: string }[] = [];
  ...
  analyzeFrameTiming(): {
    vblankTarget: string;
    frameTimeMs: number;
    timingFunctions: { function: string; description: string }[];
    syncPoints: { location: string; type: string; description: string }[];
    magicValues: { value: number; name: string; effect: string }[];
  } {
    return {
  ...
  analyzeRenderBlocking(): {
    blockingPoints: { function: string; reason: string; solution: string }[];
    initBlockers: { function: string; blocksAt: string; workaround: string }[];
    syncPrimitiveIssues: { primitive: string; problem: string; fix: string }[];
  } {
    return {
      blockingPoints: [
        {
  ...
  // ========== BUILD PROJECT ==========
  async buildProject(platform: string, mode: string): Promise<{
    success: boolean;
    preset: string;
    buildDir: string;
    commands: string[];
  ...
  // ========== LIST BUILD TARGETS ==========
  listBuildTargets(): {
    platforms: { name: string; presets: string[] }[];
    modes: string[];
    currentOS: string;
  ...
  inspectDeviceContext(): {
    tlsOffset: number;
    deviceSize: string;
    knownOffsets: { offset: number; name: string; type: string; usage: string }[];
    criticalFields: { offset: number; description: string; impact: string }[];
  } {
    return {
      tlsOffset: 1676,
  ...
  inspectRenderContext(): {
    ...
    structure: { offset: number; name: string; type: string; description: string }[];
    vtableFunctions: { index: number; function: string; description: string; blocking: boolean }[];
  } {
    return {
